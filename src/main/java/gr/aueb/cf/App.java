...
//                        .setParameter("lastname", "Μόσχος").getResultList();
...
//        criteriaQuery.select(teacherRoot).where(cb.equal(teacherRoot.get("lastname"), "Καπέτης"));
...
//        teachers4.forEach(System.out::println);
        ...
        criteriaQuery.select(teacherRoot).where(cb.equal(teacherRoot.get("lastname"), lastname));
        List<Teacher> teachers4 = em.createQuery(criteriaQuery)
                .setParameter(lastname, "Καπέτης").getResultList();
        teachers4.forEach(System.out::println);
